...
TEST_P(scheduler_policy_css_test, when_coreset0_used_then_dl_grant_is_within_bounds_of_coreset0_rbs)
{
  ...
  u.get_pcell().handle_csi_report(next_slot,
...
TEST_F(scheduler_pf_test, pf_ensures_fairness_in_dl_when_ues_have_different_channel_conditions)
{
  ...
  for (unsigned i = 0; i != 100; ++i) {
    ...
    u1.get_pcell().handle_csi_report(next_slot,
        csi_report_data{std::nullopt, std::nullopt, std::nullopt, std::nullopt, cqi_value{15U}, true});
    // Worst channel condition.
    u2.get_pcell().handle_csi_report(next_slot,
        csi_report_data{std::nullopt, std::nullopt, std::nullopt, std::nullopt, cqi_value{10U}, true});
    u3.get_pcell().handle_csi_report(next_slot,
...
TEST_P(scheduler_pf_qos_test, pf_upholds_qos_in_dl_gbr_flows)
{
  ...
  ue_with_gbr.get_pcell().handle_csi_report(next_slot,
      csi_report_data{std::nullopt, std::nullopt, std::nullopt, std::nullopt, cqi_value{15U}, true});
  ue_with_no_gbr.get_pcell().handle_csi_report(next_slot,
      csi_report_data{std::nullopt, std::nullopt, std::nullopt, std::nullopt, cqi_value{15U}, true});
