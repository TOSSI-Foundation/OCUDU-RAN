...
Two OCUDU gNBs on the same 5G core, joined by an **Xn** interface, so a UE is handed over directly between them (RAN-to-RAN signalling over Xn, then an NGAP Path Switch to the AMF to repoint the downlink). A working intra-frequency two-gNB pair ships in `configs/` ([`..._xnap_gnb1.yml`](configs/gnb_ru_liteon_tdd_n78_100mhz_xnap_gnb1.yml), [`..._xnap_gnb2.yml`](configs/gnb_ru_liteon_tdd_n78_100mhz_xnap_gnb2.yml)). Handovers trigger two ways:

| Trigger | How |
|---|---|
| **gNB-triggered (manual)** | `ho <serving_pci> <rnti> <target_pci> <target_plmn> <target_tac>` on the serving gNB console |
| **A3 event-triggered (automatic)** | set `cu_cp.mobility.trigger_handover_from_measurements: true`; the UE's A3 measurement report drives the HO |

The same serving/neighbour measurements (RSRP/RSRQ/SINR) are also exported as `cu-cp.meas_reports` KPIs over O1. Full setup and config reference: [`docs/XN_HANDOVER.md`](docs/XN_HANDOVER.md).
...
| [`docs/XN_HANDOVER.md`](docs/XN_HANDOVER.md) | Xn handover setup, the two example gNB configs, manual and A3-triggered handover, and O1 measurement KPIs. |
